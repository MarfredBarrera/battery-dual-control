function simulate_CS(x₀₀, Σ₀₀, T, L; u_noise_cov = 0.001)
    ...
    for k in 1:T
        ...
        candidate_Us[1] = apply_MPC(x₀₀, A, B, N, Q, R)[1]
        
        Threads.@threads for j in 2:L
            candidate_Us[j] = apply_MPC(x_candidates[j], A, B, N, Q, R)[1] + sqrt(u_noise_cov) .* randn(m, N)
            #sqrt(tr(Σ₀₀)) .* randn(2, N)
...
function simulate_mpc(x₀₀, Σ₀₀, T)
    ...
    for k in 1:T
        ...
        U = apply_MPC(x₀₀, A, B, N, Q, R)[1]
        u = U[:,1]
        U_rec_mpc[k] = u
        x_true = state_dynamics(x_true, u) + sqrt(W) * randn(n)
        x_true = clamp.(x_true, 0.0, 1.0)
        y = measurement_dynamics(x_true) + sqrt(V) * randn(m)
        x₀₀, Σ₀₀ = update(x₀₀, Σ₀₀, u, y, CS.eKF, mode = "measurement")
        x₀₀ = clamp.(x₀₀, 0.0, 1.0)
    end
